function [] = run_save_offline_2021_mice_kalman(mice, data_days, model_params, reps, training_trials, save_output)
...
for m = 1:length(mice)
    ...
    for d = 1:length(data_d)
        ...
        for i = 1:reps
            ...
            [results_struct] = DW_train_test_offline_func_kalman(ztrainfilt, ztestfilt, xtrain, xtest, model_params, tbt_details, [1,2],  2, []);
            
            if save_output
                % save('BMI_paper_figure_making/offline_results/res_struct_'+ mice(m) + '_' + data_d(d) + '_rep' + i + '_'+ training_trials + '.mat','results_struct');
                save('offline_kalman_results/res_struct_'+ mice(m) + '_' + data_d(d) + '_rep' + i + '_'+ training_trials + '.mat','results_struct');
            end
