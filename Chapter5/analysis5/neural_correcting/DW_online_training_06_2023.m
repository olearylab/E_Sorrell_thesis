...
% old imperfect method
% train_valid = train_invalid_sample == 0;

%% Determine correct trials. Possibly replace with function
...
%%
% keep only correct and well-behaved trials
% types_vec = [1,4,7,10]; % correct left and right normal trials in
% trial_by_trial_details. Need to check. Made a model parameter
...
% Also assume poor trials already removed.
...
% Also assume left and right trials already balanced

% Balance left and right trials. Need to check trial types. change if need
% be
% A bit arbitrary deciding which trials to discard to make the same. could
% do random, start, or end. Will do start.
...
% Optional registering to training mean. Possibly should make train_mean
% and Rfixed part of model_params, and perform this inside next function
