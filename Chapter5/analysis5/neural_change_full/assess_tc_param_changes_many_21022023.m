...
for m = 1:num_mice
    for d = 1:num_days
        if ~isempty(z_cell{m,d})
            % [full_results,full_results_overlap,full_results_both,all_params_means,mean_curve_params] = assess_tuning_curve_param_changes_alt_24062022(virmen_cell{m,d},z_cell{m,d},tbt_cell{m,d},linearise_x,nbins,boot_samps,CI_vals,xnum,false,sig_check_all{m,d});
...
%% Plotting
% 
% m_means = squeeze(mean(all_proportions,2,'omitnan'));
% m_means_overlap = squeeze(mean(all_proportions_overlap,2,'omitnan'));
% m_means_both = squeeze(mean(all_proportions_both,2,'omitnan'));
% 
% figure
% for i = 1:2
%     subplot(1,2,i)
%     bar(squeeze(mean(m_means(:,i,:))),'w','LineWidth',2)
%     hold on
%     for m = 1:num_mice
%         scatter([1,2,3],m_means(m,i,:),150,'k','filled')
%     end
%     ylim([0,1])
%     
% end
...
% for i = 1:2
%     subplot(1,2,i)
%     bar(squeeze(mean(m_means_overlap(:,i,:))),'w','LineWidth',2)
%     hold on
%     for m = 1:num_mice
%         scatter([1,2,3],m_means_overlap(m,i,:),150,'k','filled')
%     end
%     ylim([0,1])
% end
% 
...
% for i = 1:2
%     subplot(1,2,i)
%     bar(squeeze(mean(m_means_both(:,i,:))),'w','LineWidth',2)
%     hold on
%     for m = 1:num_mice
%         scatter([1,2,3],m_means_both(m,i,:),150,'k','filled')
%     end
%     ylim([0,1])
% end