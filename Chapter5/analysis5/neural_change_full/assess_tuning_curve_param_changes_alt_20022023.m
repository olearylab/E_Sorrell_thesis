...
% Significance if BMI mean outside CI for normal
...
% Is there an issue for e.g. peak location where we have discretised?
% z_peaks = zeros(length(t_types),boot_samps,size(z_binned,3));
% z_amps = zeros(length(t_types),boot_samps,size(z_binned,3));
z_widths = zeros(length(t_types),boot_samps,size(z_binned,3));

% Define width as number of bins above half peak
...
min_z = squeeze(min_z);
z_amps = max_z - min_z;
z_halves = max_z - z_amps./2;
% Need better way of getting widths. Want to get rid of loops, and ensure
% we only use the peak, not other parts that migth rise above half peak.
for i = 1:length(t_types)
    for b = 1:boot_samps
...
% Could check for no overlap of confidence interval
...
% Check if both means outside other CI
...
% add here for checking difference between left and right (or separate
% function?
...
z_halves_mean = max_z_means - z_amps_mean./2;
% Need better way of getting widths. Want to get rid of loops, and ensure
% we only use the peak, not other parts that migth rise above half peak.
for i = 1:length(t_types)
    for n = 1:size(z_binned,3)
        % z_widths_mean(n,i) = sum(squeeze(z_binned_means(:,n,i))>z_halves_mean(n,i));
...
% histogram of amplitudes, locations, and widths.
% figure
% histogram(z_amps_mean(:,1))
% figure
% histogram(z_peaks_mean(:,1))
% figure
% histogram(z_widths_mean(:,1))
if plot_res
    % Now changes
    figure
...
%% Need some plotting checks!
% Maybe move to new function

% Plot number of neurons with each kind of change

% First just each change separately
if plot_res
    figure
    for i = 1:2
        subplot(1,2,i)
