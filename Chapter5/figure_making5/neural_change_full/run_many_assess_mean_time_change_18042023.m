...
%% Alternative using binned averages
...
% % end
% % nbins = 20;
% % x_binned = zeros(2,nbins);
% % for t = 1:2
% %     [binned,edges] = discretize(all_x{t},nbins);
% %     for j = 1:nbins
% %         if sum(binned==j) == 1
% %             x_binned(t,j) = all_means{t}(binned==j);
% %         else
% %             x_binned(t,j) = mean(all_means{t}(binned==j));
% %         end
% %     end
% %     centres = (edges(2:end)+edges(1:end-1))/2;
% %     plot(centres,x_binned(t,:),'LineWidth',3,'Color',colour_vec(t,:))
% % end
% % xlabel("Trial Number")
% % ylabel("Mean Normalised Activity (a.u.)")
% % title("Mean Activity Over Time")
