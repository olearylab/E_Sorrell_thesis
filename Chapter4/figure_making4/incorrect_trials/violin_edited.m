...
if isempty(find(strcmp(varargin,'Support')))==0
    vlims = varargin{find(strcmp(varargin,'Support'))+1};
end
%%
if size(fc,1)==1
...
for i=1:size(Y,2)
    ...
    if isempty(b)==0
        if isempty(vlims)==0
            [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'Support',vlims);
        else
            [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i));
        end
    elseif isempty(b)
        if isempty(vlims)==0
            [f, u, bb]=ksdensity(Y{i},'Support',vlims);
        else
