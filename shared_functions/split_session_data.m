...
% zdata is whole session image data
...
% Now added consideration of removal of poor trials (including whatever
% the current criterion for this is), otherwise may be less and unbalanced
% in the end.

% Can also determine number of testing trials instead of training trials

types_vec = [1,4,7,10]; % correct trial numbers for types 1,2,3,4
...
% Currently not bothering with balancing by test. Could add easily.
if by_test
    cur_trials = find(ismember(tbt_details(3,:),types_vec));
